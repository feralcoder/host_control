...
[[ $SKIP_IP == "true" ]] || {
  ...
  ilo_boot_set_onetimeboot_these_hosts ip "$(group_logic_intersection "$ILO4_HOSTS" "$STACK_HOSTS")"
  ilo_power_on_these_hosts "$(group_logic_intersection "$ILO4_HOSTS" "$STACK_HOSTS")"
...
ssh_control_run_as_user_these_hosts root "$PRE_CMD; $FIND_DRIVE_CMD; $DD_CMD && echo true > /tmp/stack_drive_hosed_$$" "$STACK_HOSTS"

ssh_control_run_as_user_these_hosts root "if [[ \`cat /tmp/stack_drive_hosed_$$\` == '' ]]; then echo \`hostname\` NEEDS MANUAL HOSING.; fi" "$STACK_HOSTS"
