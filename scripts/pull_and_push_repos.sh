...
ssh_control_run_as_user_these_hosts root "chown -R cliff:cliff /$HOME/CODE" $HOST
ssh_control_run_as_user_these_hosts cliff "chown -R cliff:cliff /$HOME/CODE" $HOST
ssh_control_sync_as_user_these_hosts cliff /$HOME/.git_password /$HOME/.git_password $HOST


for REPO in bootstrap-scripts  host_control  kolla-ansible  workstation  workstation.wiki; do
  CODE_PATH=/$HOME/CODE/feralcoder/$REPO
