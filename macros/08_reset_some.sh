...
THESE_HOSTS="kgn neo bmn lmn mtn"
...
boot_to_target () {
  ...
  os_control_boot_to_target_installation_these_hosts $TARGET "$THESE_HOSTS" || return 1
  ssh_control_wait_for_host_up_these_hosts "$THESE_HOSTS" || return 1
  os_control_assert_hosts_booted_target $TARGET "$THESE_HOSTS" || {
...
$MACRO_DIR/09_reset_everything.sh $RESTORE_FROM a "$THESE_HOSTS"  || fail_exit "09_reset_everything.sh $RESTORE_FROM a \"$THESE_HOSTS\""

echo; echo "BOOTING TO DEFAULT OS ON $THESE_HOSTS"
